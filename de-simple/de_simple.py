...
class DE_SimplE(torch.nn.Module):
    def __init__(self, dataset, params):
        ...
        self.ent_embs_h = nn.Embedding(dataset.numEnt(), params.s_emb_dim).to(self.params.device)
        self.ent_embs_t = nn.Embedding(dataset.numEnt(), params.s_emb_dim).to(self.params.device)
        self.rel_embs_f = nn.Embedding(dataset.numRel(), params.s_emb_dim + params.t_emb_dim).to(self.params.device)
        self.rel_embs_i = nn.Embedding(dataset.numRel(), params.s_emb_dim + params.t_emb_dim).to(self.params.device)
    ...
    def create_time_embedds(self):
        ...
        self.m_freq_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.m_freq_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.d_freq_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.d_freq_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.y_freq_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.y_freq_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)

        # phi embeddings for the entities
        self.m_phi_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.m_phi_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.d_phi_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.d_phi_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.y_phi_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.y_phi_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)

        # frequency embeddings for the entities
        self.m_amps_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.m_amps_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.d_amps_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.d_amps_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.y_amps_h = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
        self.y_amps_t = nn.Embedding(self.dataset.numEnt(), self.params.t_emb_dim).to(self.params.device)
